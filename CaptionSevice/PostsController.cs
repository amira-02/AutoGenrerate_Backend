...
public class PostsController : ControllerBase
{
    ...
    public async Task<IActionResult> Chat([FromBody] ChatDto dto)
    {
        ...
        try
        {
            var response = await client.PostAsJsonAsync(
                "http://localhost:5678/webhook-test/chatbot",   // ← production URL (no webhook-test)
                new
                {
                    message = dto.Message ?? "",
                    topic = dto.Topic ?? "",
    ...
    public async Task<IActionResult> SaveCaption([FromBody] SaveCaptionDto dto)
    {
        try
        {
            ...
            {
                post.Captions.Add(new AutoGenerate.CaptionService.Models.Caption
                {
                    Content = dto.Caption,
        ...
        var posts = await _db.Posts
            ...
            .Select(p => new
            {
                ...
                tone = p.Captions
                    ...
                    .FirstOrDefault(),
                status = "draft"   // adapte si tu as un champ status dans Post
            })
            .ToListAsync();
    ...
    public async Task<IActionResult> GetPost(int id)
    {
        ...
        var post = await _db.Posts
            ...
            .Select(p => new
            {
                ...
                hashtags = p.Hashtags,
                createdAt = p.CreatedAt,
                captions = p.Captions.Select(c => new
                {
                    id = c.Id,
                    content = c.Content,
                    tone = c.ToneOfVoice,
